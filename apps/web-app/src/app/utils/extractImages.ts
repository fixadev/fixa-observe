    ...
    try {
      ...
      const credentials = new ServicePrincipalCredentials({
        ...
        try {
            ...
            const figureFiles = Object.keys(zipContent.files).filter(filename => 
              filename.startsWith('figures/') && filename.toLowerCase().endsWith('.png')
            );
            ...
            for (const filename of figureFiles) {
              try {
                ...
                if (blobContent) {
                  ...
                  const fileContent = new File([blobContent], filename, { type: blobContent.type });
      ...
      return s3objects.sort((a, b) => parseInt(a.index) - parseInt(b.index)).map(obj => obj.url);
