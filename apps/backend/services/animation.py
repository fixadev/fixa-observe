...
def generate_animation(manim_code, output_file):
    ...
    try:
        scene.render()
        generated_files = [f for f in os.listdir(config.media_dir) if f.endswith('.mp4')]
        if not generated_files:
            raise FileNotFoundError("No MP4 file was generated by Manim")
        
        generated_file = os.path.join(config.media_dir, generated_files[0])
